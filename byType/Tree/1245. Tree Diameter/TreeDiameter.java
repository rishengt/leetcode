...
public class TreeDiameter {
    public int treeDiameter(int[][] edges) {
        ...
        int[] start = dfs(graph,visited,0);
        int res[] = dfs(graph,visited,start[0]);
        return res[1];
    }

    public int[] dfs(List<Integer>[] graph, boolean[] visited, int start){
        visited[start] = true;
        int dist = 0;
        int node = start;
        for(int child: graph[start]){
            if(visited[child])continue;
            int[] returnedValues = dfs(graph,visited,child);
