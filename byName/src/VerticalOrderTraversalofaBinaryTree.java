    ...
    public List<List<Integer>> verticalTraversal(TreeNode root) {
        ...
        while(!queue.isEmpty()){
            ...
            for(int i = 0; i<size; i++){
                ...
                map.get(temp.x).add(temp);
                min = Math.min(temp.x,min);
                max = Math.max(temp.x,max);
                if(temp.node.left !=null){
                    queue.offer(new Pair(temp.node.left,temp.x-1,temp.y-1));
                }
                if(temp.node.right !=null){
                    queue.offer(new Pair(temp.node.right, temp.x+1, temp.y-1));
                }
            }
        }
